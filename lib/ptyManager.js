...
function attachSession(sessionName, ws, cols = 80, rows = 24, ntfyTopicArg = '') {
  ...
  proc.onData((rawData) => {
    ...
    const checkAutoYes = () => {
      ...
      // 学習用: 明確なプロンプトパターンのみ記録（誤学習防止）
      // ?で終わる、(y/n)を含む、選択肢(❯ 1.)がある等、かつ罫線やコードではない
      const lastLine = checkText.split('\n').filter(l => l.trim()).pop() || '';
      const looksLikePrompt = /[?？]\s*$/.test(lastLine) || /\(y\/n\)|\[y\/n\]/i.test(lastLine) || /❯\s+\d+\./.test(lastLine);
      const looksLikeNoise = /^[─━╌╭╮┌┐└┘│┃⏺⎿\s]+$/.test(lastLine) || /^\s*(const|let|var|function|import|export|if|for|return)\s/.test(lastLine);
      if (looksLikePrompt && !looksLikeNoise && lastLine.length > 5) {
        pendingPrompt = { text: lastLine, timestamp: Date.now() };
      }
    };
    checkAutoYes();
