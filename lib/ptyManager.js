...
function attachSession(sessionName, ws, cols = 80, rows = 24, ntfyTopicArg = '', ntfyIconUrl = '') {
  ...
  proc.onData((data) => {
    ...
    if (ntfyTopic) {
      ...
      doneTimer = setTimeout(() => {
        if (hadOutput) {
          sendNtfy(ntfyTopic, `⚡ ${sessionName}`, 'ちゅどーん！完了したっちゃ！', 3, ntfyIconUrl);
          hadOutput = false;
        }
      }, 3000);
      ...
      if (!questionCooldown) {
        ...
        for (const pattern of QUESTION_PATTERNS) {
          if (pattern.test(plain)) {
            sendNtfy(ntfyTopic, `❓ ${sessionName}`, '返事が必要っちゃ！確認してっちゃ！', 4, ntfyIconUrl);
    ...
    if (autoYes && !autoYesCooldown) {
      // ANSI エスケープ除去して即パターンチェック（デバウンスなし）
      const plain = data
        .replace(/\x1B\][^\x07\x1B]*(\x07|\x1B\\)/g, '')
        .replace(/[\x1B\x9B][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><~]/g, '')
        .replace(/[\x00-\x09\x0B-\x0C\x0E-\x1F\x7F]/g, '');
      for (const { pattern, response } of AUTO_YES_PATTERNS) {
        if (pattern.test(plain)) {
          ...
          autoYesCooldownTimer = setTimeout(() => { autoYesCooldown = false; }, 3000);
          break;
        }
      }
    }
