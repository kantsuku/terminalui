...
function attachSession(sessionName, ws, cols = 80, rows = 24, ntfyTopicArg = '') {
  ...
  // デバウンス方式AutoYES: 出力が止まってからチェック
  // 出力中は誤爆しない。プロンプトは出力が止まるので検知できる。
  let recentBuffer = '';
  let autoYesCheckTimer = null;
  const BUFFER_MAX = 500;
  const CHECK_DELAY_MS = 300; // 300ms無音でプロンプトと判定
  let lastMatchTime = 0;
  ...
  proc.onData((rawData) => {
    ...
    // デバウンス: 300ms出力が止まったらAutoYESチェック
    clearTimeout(autoYesCheckTimer);
    if (autoYesMode && !autoYesCooldown) {
      autoYesCheckTimer = setTimeout(() => checkAutoYes(), CHECK_DELAY_MS);
    }
    ...
    // AUTO-YES チェック（デバウンスタイマーから呼ばれる — 出力停止後のみ）
    const checkAutoYes = () => {
