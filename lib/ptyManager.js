...
const AUTO_YES_PATTERNS = [
  // ── Claude Code 固有 ──────────────────────────────────────────────────────
  // ★ 具体的なパターンを先に、汎用パターンを後に配置 ★
  // Claude Code 編集確認（Esc to cancel + Tab to amend が出る）→ shift+tab で allow all
  { pattern: /do you want to make this edit/i,   response: '\x1b[Z',   safe: true },
  ...
  // ★ 汎用 "do you want to" は「make this edit」を含まない場合のみ
  { pattern: /do you want to(?! make this edit)/i, response: '\r',      safe: true },
