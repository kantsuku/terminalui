...
function attachSession(sessionName, ws, cols = 80, rows = 24, ntfyTopicArg = '') {
  ...
  // 直近の出力を保持する安全なミニバッファ（最大500バイト）
  // 50ms無音でクリア → 古いテキストが残り続けない
  let recentBuffer = '';
  ...
  proc.onData((rawData) => {
    ...
    // ミニバッファに蓄積（直近500バイトのみ保持、50ms無音でクリア）
    recentBuffer = (recentBuffer + plain).slice(-BUFFER_MAX);
    clearTimeout(bufferFlushTimer);
    bufferFlushTimer = setTimeout(() => { recentBuffer = ''; }, BUFFER_FLUSH_MS);
    ...
    const checkAutoYes = () => {
      ...
      for (const { pattern, response, safe } of allPatterns) {
        if (pattern.test(checkText) || pattern.test(recentBuffer)) {
