...
const AUTO_YES_PATTERNS = [
  ...
  { pattern: /framework.*:/i,                    response: '\r',        safe: true },
  { pattern: /variant.*:/i,                      response: '\r',        safe: true },
  ...
  { pattern: /確認/,                              response: 'y\r',       safe: true },
  ...
  // ── 汎用デフォルト値プロンプト ────────────────────────────────────────────
  { pattern: /\[[^\]]+\]:\s*$/,                  response: '\r',        safe: true },
  { pattern: /\([^)]+\)\s*$/,                    response: '\r',        safe: true },
...
function attachSession(sessionName, ws, cols = 80, rows = 24, ntfyTopicArg = '') {
  ...
  proc.onData((rawData) => {
    ...
    const checkAutoYes = () => {
      ...
      // どのパターンにもマッチしなかった → 出力がプロンプトっぽいか判定
      // ?で終わる、y/nを含む、選択肢がある等
      if (outputBuffer && /[?？]\s*$|\(y\/n\)|\[y\/n\]|›\s+\d+\./i.test(outputBuffer)) {
        pendingPrompt = { text: outputBuffer.slice(-500), timestamp: Date.now() };
        console.log('[AutoYes:Learn] Unmatched prompt detected, waiting for manual response...');
      }
    };
    checkAutoYes();
  ...
  return {
    proc,
    write: (data) => {
      // 学習: autoYES ON + 未対応プロンプトがある + ユーザーが手動応答した
      if (autoYesMode && pendingPrompt) {
        const elapsed = Date.now() - pendingPrompt.timestamp;
        if (elapsed < LEARN_WINDOW_MS) {
          // y, yes, Enter などの肯定的応答のみ学習対象
          const resp = data.replace(/\r?\n?$/, '');
          if (/^(y|yes|Y|YES|\r)$/i.test(data) || data === '\r' || data === '\x1b[Z') {
            const learned = learnPattern(pendingPrompt.text, data);
            if (learned && ws.readyState === 1) {
