...
const AUTO_YES_PATTERNS = [
  ...
  // Claude Code フィードバック（optional → Escでスキップ）
  { pattern: /How is Claude doing/i,             response: '\x1b' },   // Esc でスキップ
  { pattern: /\(optional\)/i,                    response: '\x1b' },   // optional系はEscでスキップ
  // Claude Code › Yes / No 選択プロンプト
  { pattern: /›\s*No\s*\/\s*Yes/i,              response: '\x1b[C\r' }, // → で Yes に移動して Enter
  { pattern: /›\s*Yes\s*\/\s*No/i,              response: '\r' },       // 既にYes → Enter
  ...
  { pattern: /RETURN\/ENTER to continue/i,       response: '\r' },
  { pattern: /press any key/i,                   response: '\r' },
  { pattern: /==> This script will install/i,    response: '\r' },

  // ── gh CLI 認証・選択 ──────────────────────────────────────────────────
  { pattern: /How would you like to authenticate/i, response: '\r' },
  { pattern: /Login with a web browser/i,        response: '\r' },
  { pattern: /What account do you want/i,        response: '\r' },
  { pattern: /What is your preferred protocol/i, response: '\r' },
  { pattern: /Upload SSH public key/i,           response: '\r' },
  { pattern: /Title for your SSH key/i,          response: '\r' },
  { pattern: /Where do you use GitHub/i,         response: '\r' },
  { pattern: /Authenticate Git with/i,           response: '\r' },
  { pattern: /Press Enter to open/i,             response: '\r' },
  // gh CLI の ? で始まるインタラクティブ選択（デフォルト選択でEnter）
  { pattern: /^\?\s+\w/m,                        response: '\r' },
  ...
  { pattern: /project name/i,                    response: '\r' },

  // ── next.js create-next-app ─────────────────────────────────────────────
  { pattern: /Would you like to use TypeScript/i,    response: '\r' },
  { pattern: /Would you like to use ESLint/i,        response: '\r' },
  { pattern: /Would you like to use Tailwind/i,      response: '\r' },
  { pattern: /Would you like.*src.*directory/i,      response: '\r' },
  { pattern: /Would you like to use App Router/i,    response: '\r' },
  { pattern: /Would you like to use Turbopack/i,     response: '\r' },
  { pattern: /Would you like to customize/i,         response: '\r' },
  { pattern: /Would you like to use React Compiler/i, response: '\r' },
  { pattern: /import alias/i,                        response: '\r' },

  // ── Supabase CLI ────────────────────────────────────────────────────────
  { pattern: /Generate VS Code settings/i,       response: '\r' },
  { pattern: /Link.*project/i,                   response: '\r' },

  // ── ターミナルページャー（less/more）──────────────────────────────────────
  { pattern: /--More--/,                         response: ' ' },

  // ── esc to interrupt（Claude の処理中断ではないので無視）─────────────────
  // { pattern: /esc to interrupt/i } は意図的に除外（作業中断させたくない）
