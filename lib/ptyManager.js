...
// ══════════════════════════════════════════════════════════════════════════════
// AUTO-YES ハイブリッド方式パターン
// safe: true      → 半自動モードでも自動応答（日常的な確認）
// safe: false     → 全自動モードのみ自動応答（破壊的・重要判断）
// delayedOnly: true → 遅延チェック(100ms後)のみ。出力停止後にだけマッチ。
//                     Claude出力中は誤爆しない（タイマーがリセットされ続ける）
// ══════════════════════════════════════════════════════════════════════════════
...
const AUTO_YES_PATTERNS = [
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // 遅延専用パターン（delayedOnly: true）
  // 500ms出力停止後にのみチェック。Claude出力中は絶対に誤爆しない。
  // 以前「Claude出力に誤爆」として削除したパターンを安全に復活。
  // ══════════════════════════════════════════════════════════════════════════

  // ── Claude Code TUI（遅延専用）──────────────────────────────────────────
  { pattern: /Esc to cancel/i,                    response: '\r',        safe: true,  delayedOnly: true },
  { pattern: /do you want to make this edit/i,    response: 'y\r',       safe: true,  delayedOnly: true },
  { pattern: /trust.*folder/i,                    response: '\r',        safe: true,  delayedOnly: true },

  // ── 汎用確認（遅延専用）────────────────────────────────────────────────
  { pattern: /do you want to/i,                   response: 'y\r',       safe: true,  delayedOnly: true },
  { pattern: /would you like/i,                   response: 'y\r',       safe: true,  delayedOnly: true },
  { pattern: /shall i/i,                          response: 'y\r',       safe: true,  delayedOnly: true },
  { pattern: /press enter/i,                      response: '\r',        safe: true,  delayedOnly: true },
  { pattern: /proceed\?/i,                        response: 'y\r',       safe: true,  delayedOnly: true },
  { pattern: /continue\?/i,                       response: 'y\r',       safe: true,  delayedOnly: true },
  { pattern: /confirm\?/i,                        response: 'y\r',       safe: false, delayedOnly: true },
  { pattern: /accept\?/i,                         response: 'y\r',       safe: true,  delayedOnly: true },
  { pattern: /allow.*\?/i,                        response: 'y\r',       safe: true,  delayedOnly: true },
  { pattern: /approve\?/i,                        response: 'y\r',       safe: true,  delayedOnly: true },
  { pattern: /install.*anyway/i,                  response: 'y\r',       safe: true,  delayedOnly: true },
  { pattern: /what would you like/i,              response: '\r',        safe: true,  delayedOnly: true },
  { pattern: /which.*method/i,                    response: '\r',        safe: true,  delayedOnly: true },
  { pattern: /select.*option/i,                   response: '\r',        safe: true,  delayedOnly: true },

  // ── 選択UI（遅延専用）──────────────────────────────────────────────────
  { pattern: /›\s*Yes\s*\/\s*No/i,                response: '\r',        safe: true,  delayedOnly: true },
  { pattern: /›\s*No\s*\/\s*Yes/i,                response: '\x1b[B\r',  safe: true,  delayedOnly: true },  // ↓で選択してEnter
  { pattern: /^\?\s+\w/m,                         response: '\r',        safe: true,  delayedOnly: true },  // inquirer.js

  // ── create-app / CLI ウィザード（遅延専用）─────────────────────────────
  { pattern: /framework.*:/i,                     response: '\r',        safe: true,  delayedOnly: true },
  { pattern: /variant.*:/i,                       response: '\r',        safe: true,  delayedOnly: true },
  { pattern: /project name/i,                     response: '\r',        safe: true,  delayedOnly: true },
  { pattern: /visibility.*private|public/i,       response: '\r',        safe: true,  delayedOnly: true },
  { pattern: /Link.*project/i,                    response: '\r',        safe: true,  delayedOnly: true },

  // ── 汎用プロンプト末尾（遅延専用）─────────────────────────────────────
  { pattern: /\?\s*$/,                            response: '\r',        safe: true,  delayedOnly: true },
  { pattern: /\[[^\]]+\]:\s*$/,                   response: '\r',        safe: true,  delayedOnly: true },  // [default]: 形式
  { pattern: /\([^)]+\)\s*$/,                     response: '\r',        safe: true,  delayedOnly: true },  // (option) 形式

  // ── ページャー（遅延専用）──────────────────────────────────────────────
  { pattern: /--More--/,                          response: ' ',         safe: true,  delayedOnly: true },

  // ── 日本語（遅延専用）──────────────────────────────────────────────────
  { pattern: /確認しますか/,                       response: 'y\r',       safe: true,  delayedOnly: true },
  { pattern: /選択してください/,                   response: '\r',        safe: true,  delayedOnly: true },
  { pattern: /入力してください/,                   response: '\r',        safe: true,  delayedOnly: true },
];
...
function attachSession(sessionName, ws, cols = 80, rows = 24, ntfyTopicArg = '') {
  ...
  // ハイブリッド方式AutoYES:
  // 即時: 現チャンクのみでマッチ（元の動作、バッファ不使用で安全）
  // 遅延: 100ms後にバッファでマッチ（チャンク分割キャッチ用）
  let recentBuffer = '';
  let autoYesCheckTimer = null;
  const BUFFER_MAX = 500;
  const CHECK_DELAY_MS = 500; // 500ms出力停止でプロンプトと判定（100msではClaude描画隙間で誤爆）
  let lastMatchTime = 0;
  ...
  proc.onData((rawData) => {
    ...
    // ミニバッファに蓄積（直近500バイトのみ保持）
    recentBuffer = (recentBuffer + plain).slice(-BUFFER_MAX);

    // AUTO-YES マッチ（即時: 現チャンク / 遅延: バッファ）
    const matchAutoYes = (text, isDelayed = false) => {
      if (!autoYesMode || autoYesCooldown) return;
      if (Date.now() - lastMatchTime < AUTO_YES_COOLDOWN_MS) return;
      for (const { pattern, response, safe, delayedOnly } of AUTO_YES_PATTERNS) {
        if (delayedOnly && !isDelayed) continue; // 即時チェックではスキップ
    ...
    // ハイブリッド方式AutoYES:
    // 1. 即時: 現チャンクのみでマッチ（バッファ不使用→誤爆なし）
    // 2. 遅延: 100ms後にバッファでマッチ（チャンク分割キャッチ用）
    if (autoYesMode && !autoYesCooldown) {
      matchAutoYes(checkText, false);  // 即時チェック（現チャンクのみ、delayedOnlyスキップ）
      clearTimeout(autoYesCheckTimer);
      autoYesCheckTimer = setTimeout(() => matchAutoYes(recentBuffer, true), CHECK_DELAY_MS);
    }
