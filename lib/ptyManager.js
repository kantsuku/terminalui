...
const AUTO_YES_PATTERNS = [
  ...
  { pattern: /proceed\s*\?/i,                     response: 'y\r',       safe: true,  delayedOnly: true },
  { pattern: /continue\s*\?\s*$/im,               response: 'y\r',       safe: true,  delayedOnly: true },
  { pattern: /confirm\s*\?/i,                     response: 'y\r',       safe: false, delayedOnly: true },
...
function attachSession(sessionName, ws, cols = 80, rows = 24, ntfyTopicArg = '') {
  ...
  const CHECK_DELAY_MS = 100; // 100ms後にバッファチェック
  let lastMatchTime = 0;
