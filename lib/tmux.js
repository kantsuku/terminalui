...
async function createSession(name, command, systemPrompt) {
  ...
  // プロセス終了時にセッションを残す（勝手に消えるのを防止）
  await execFileAsync(TMUX, ['set-option', '-t', sessionName, 'remain-on-exit', 'on']).catch((e) => { console.warn('[tmux] remain-on-exit error:', e.message); });
  return sessionName;
}
